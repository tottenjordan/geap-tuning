...
| **RLFT reward ranking** | Which reward shape best teaches an absent output behavior (ranks `format_rate`, since RLFT's only base saturates correctness) | `examples/run_doe_rlft_reward_ranking.py` | `notebooks/16_doe_reward_ranking.ipynb` | [rlft-reward-ranking](rlft-reward-ranking/README.md) (re-targeted to `gemini-3.5-flash`; format gate passed, jobs launching) |
