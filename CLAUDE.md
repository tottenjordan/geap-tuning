...
- **Supervised fine-tuning (SFT)** — the core method; teaches a new skill/behavior from labeled JSONL examples (`contents` records). Implemented (`sft/`). **Multimodal (image) SFT** is also demonstrated (`sft_vision/` — oral-disease image classification, ported from jswortz/dental-fine-tune-26): records carry a `fileData` image part, but the tune call is **identical** to text SFT so it reuses `sft.tune.launch_sft_job` (no new tune module). Dataset via optional `vision` group (`kagglehub`, lazy-imported; needs `KAGGLE_API_TOKEN`); demoed by `examples/run_sft_vision.py`/`notebooks/09_sft_vision.ipynb` with a hyperparameter sweep → val-select → test. See [docs/notes/multimodal-sft.md](docs/notes/multimodal-sft.md).
